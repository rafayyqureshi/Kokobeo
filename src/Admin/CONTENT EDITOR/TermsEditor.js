...
const TermsEditor = () => {
  ...
  const SocialMediaModal = ({ isOpen, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"  style={{ textAlign: 'left' }}>
        <div className="bg-white rounded-xl w-full max-w-2xl p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-semibold">Social Media URLs</h2>
            <button onClick={onClose}><X className="h-5 w-5" /></button>
          </div>
          <div className="space-y-4">
            <div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-gray-500 text-sm">https://kokobeo.com</span>
                  <input 
                    type="text" 
                    value={pageData.url} 
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={(e) => setPageData({...pageData, layout: e.target.value})} 
                  className="w-full px-3 py-2 border rounded-lg text-sm"
                >
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="flex rounded-md overflow-hidden border">
                    <button 
                      className={`px-3 py-1 text-xs ${contentType === 'text' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100'}`}
                      onClick={() => setContentType('text')}
                    >
                      Text
                    </button>
                    <button 
                      className={`px-3 py-1 text-xs ${contentType === 'html' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100'}`}
                ...
                <div className="flex items-start gap-2">
                  {contentType === 'html' && <Code className="h-5 w-5 text-gray-400 mt-2" />}
                  <textarea 
                    value={pageData.content} 
                    onChange={(e) => setPageData({...pageData, content: e.target.value})} 
                    rows={10} 
                    className={`w-full px-3 py-2 border rounded-lg text-sm ${contentType === 'html' ? 'font-mono' : ''}`} 
                    placeholder={contentType === 'html' ? "Enter HTML content" : "Enter page content"} 
                  />
                </div>
                {contentType === 'html' && (
                  <p className="text-xs text-gray-500 mt-1">You can use HTML tags to format your content.</p>
                )}
              </div>
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <textarea 
                      ...
                      onChange={(e) => setPageData({...pageData, metadata: {...pageData.metadata, description: e.target.value}})} 
                      rows={2} 
                      className="w-full px-3 py-2 border rounded-lg text-sm" 
                  ...
                  <div>
                    ...
                    <input 
                      type="text" 
                      value={pageData.metadata.keywords} 
                      onChange={(e) => setPageData({...pageData, metadata: {...pageData.metadata, keywords: e.target.value}})} 
                      className="w-full px-3 py-2 border rounded-lg text-sm" 
  ...
  const ContentEditor = ({ documentType }) => {
    ...
    const renderPageManager = () => {
      ...
      return (
        ...
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-medium">Page Management</h3>
          <Button onClick={() => setShowPageEditor(true)}>
            <Plus className="h-4 w-4 mr-2" />Create New Page
          </Button>
        </div>
        <div className="grid gap-4">
          {Object.entries(currentPages).map(([key, page]) => (
            <Card key={key} className="p-4">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <h4 className="font-medium">{page.title}</h4>
                  <div className="text-sm text-gray-500 flex items-center gap-2">
                    <Link className="h-4 w-4" />{page.url}
                  </div>
                </div>
              ...
              <div className="mt-4 flex gap-2">
                <Badge variant="outline" className="text-sm">{page.layout} layout</Badge>
                <Badge className={`text-sm ${page.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                  {page.status}
                </Badge>
              </div>
            </Card>
              ...
              <div className="space-y-4">
                {(documentType === 'customerInfo' || documentType === 'professionalInfo') && (
                  <div className="mb-4">
                    ...
                    <select className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" value={selectedCountryContent} onChange={(e) => setSelectedCountryContent(e.target.value)}>
                ...
                {Object.entries(localContent).map(([key, section]) => (
                  <div key={key} className="border rounded-lg overflow-hidden">
                    <button onClick={() => setExpandedSection(expandedSection === key ? null : key)} className="w-full flex items-center justify-between p-3 sm:p-4 bg-gray-50 hover:bg-gray-100">
                      <div className="flex items-center gap-2">
                        <FileText className="h-4 w-4" />
                        <span className="font-medium text-sm sm:text-base">{section.title}</span>
                      </div>
                      <ChevronDown className={`h-4 w-4 transform transition-transform ${expandedSection === key ? 'rotate-180' : ''}`} />
                    </button>
                    {expandedSection === key && (
                      <div className="p-3 sm:p-4 space-y-4">
                        <div>
                          ...
                          <input 
                            ...
                            className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" 
                          />
                        </div>
                        <div>
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <textarea 
                              ...
                              onChange={(e) => handleContentChange(key, 'content', e.target.value)} 
                              rows={4} 
                              className={`w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg resize-none text-sm ${contentType === 'html' ? 'font-mono' : ''}`} 
                            />
                          </div>
                          {contentType === 'html' && (
                            <p className="text-xs text-gray-500 mt-1">HTML formatting is supported in this field.</p>
                          )}
                        </div>
                        <div className="flex flex-col sm:flex-row justify-end gap-2">
                          ...
                          <Button size="sm" className="w-full sm:w-auto">
                            <Save className="h-3 w-3 sm:h-4 sm:w-4 mr-2" />Save
                          </Button>
                        </div>
                      </div>
                    )}
                ...
                <Button variant="outline" onClick={addNewSection} className="w-full mt-4 text-sm sm:text-base">
          ...
          return (
            <Card className="p-4 sm:p-6">
              <div className="space-y-6">
                <div>
                  <h3 className="text-base sm:text-lg font-medium mb-4">Document Settings</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Default Language</label>
                      <select className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm">
                ...
                {(documentType === 'customerInfo' || documentType === 'professionalInfo') && (
                  <div>
                    <h3 className="text-base sm:text-lg font-medium mb-4">Page Display Settings</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Menu Placement</label>
                        <select className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm">
                          {menuOptions.map(option => <option key={option.id} value={option.id}>{option.name}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">User Access</label>
                        <select className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm">
                          {userAccessOptions.map(option => <option key={option.id} value={option.id}>{option.name}</option>)}
                        </select>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <input type="number" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" placeholder="1" min="1" />
                      </div>
                    </div>
                  </div>
                )}
                <div>
                  <h3 className="text-base sm:text-lg font-medium mb-4">Access Control</h3>
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">Review Permissions</span>
                      </div>
                      <Button variant="outline" size="sm" className="mt-2 sm:mt-0">Configure</Button>
                    </div>
                  </div>
                </div>
                <div>
                  <h3 className="text-base sm:text-lg font-medium mb-4">Version Control</h3>
                  <div className="space-y-3">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-2">
                      ...
                      <Button variant="outline" size="sm" className="mt-2 sm:mt-0">View History</Button>
                    </div>
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center mt-2 sm:mt-0">
                        <span className="text-sm text-gray-500 mr-2">Every 5 minutes</span>
            ...
            <Card className="p-4 sm:p-6">
              <div className="space-y-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
                  <h3 className="text-base sm:text-lg font-medium mb-2 sm:mb-0">Available Translations</h3>
                  <Button className="w-full sm:w-auto text-sm">
                    <Plus className="h-4 w-4 mr-2" />Add Translation
                  </Button>
                </div>
                <div className="space-y-3">
                  {['French', 'Spanish', 'German'].map((language, index) => (
                    <div key={index} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs sm:text-sm text-gray-500">Last updated: 2 days ago</div>
                        </div>
                      </div>
    ...
    return (
      <div className="space-y-6"  style={{ textAlign: 'left' }}>
        <div className="flex gap-2">
          <Button onClick={() => setContentTab('content')} variant={contentTab === 'content' ? 'default' : 'outline'}>Content</Button>
          <Button onClick={() => setContentTab('settings')} variant={contentTab === 'settings' ? 'default' : 'outline'}>Settings</Button>
          <Button onClick={() => setContentTab('translations')} variant={contentTab === 'translations' ? 'default' : 'outline'}>Translations</Button>
        </div>
        {renderTabContent()}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start">
        <div className="mb-4 sm:mb-0">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-base sm:text-lg font-semibold">Version {version.version}</h3>
            <Badge className={`text-xs sm:text-sm ${version.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
              {version.status.charAt(0).toUpperCase() + version.status.slice(1)}
            </Badge>
            <Badge variant="outline" className="text-xs sm:text-sm">
              {documentTypes.find(t => t.id === version.type)?.name || version.type.toUpperCase()}
            </Badge>
            {version.country && <Badge className="text-xs sm:text-sm bg-purple-100 text-purple-700">{version.country}</Badge>}
          </div>
      ...
      {version.status === 'published' && (
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="text-xs sm:text-sm text-gray-500">Publish Date</div>
            <div className="font-medium text-sm sm:text-base">{version.publishDate}</div>
          </div>
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="text-xs sm:text-sm text-gray-500">Effective Date</div>
            <div className="font-medium text-sm sm:text-base">{version.effectiveDate}</div>
          </div>
        </div>
      )}
      {(version.type === 'customerInfo' || version.type === 'professionalInfo') && (
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="text-xs sm:text-sm text-gray-500">Menu Placement</div>
            <div className="font-medium text-sm sm:text-base">
              {menuOptions.find(m => m.id === version.menuPlacement)?.name || 'Not Set'}
            </div>
          </div>
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="text-xs sm:text-sm text-gray-500">User Access</div>
            <div className="font-medium text-sm sm:text-base">
              {userAccessOptions.find(u => u.id === version.userAccess)?.name || 'Not Set'}
            </div>
          </div>
        </div>
      )}
      <div className="mt-4 sm:mt-6">
        <h4 className="font-medium mb-3 text-sm sm:text-base">Content Sections</h4>
        <div className="space-y-3">
          {version.sections.map(section => (
            <div key={section.id} className="p-3 bg-gray-50 rounded-lg">
              <h5 className="font-medium text-sm sm:text-base">{section.title}</h5>
              <p className="text-xs sm:text-sm text-gray-600 mt-1 line-clamp-2">{section.content}</p>
            </div>
          ))}
        </div>
      </div>
      <div className="mt-4 sm:mt-6">
        <h4 className="font-medium mb-3 text-sm sm:text-base">Review Status</h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
            <div className="text-xs sm:text-sm">Legal Review</div>
            <Badge className={`text-xs sm:text-sm ${version.reviewStatus.legal === 'approved' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
              {version.reviewStatus.legal.charAt(0).toUpperCase() + version.reviewStatus.legal.slice(1)}
            </Badge>
          </div>
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
            <div className="text-xs sm:text-sm">Compliance Review</div>
            <Badge className={`text-xs sm:text-sm ${version.reviewStatus.compliance === 'approved' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
              {version.reviewStatus.compliance.charAt(0).toUpperCase() + version.reviewStatus.compliance.slice(1)}
            </Badge>
          </div>
        </div>
      </div>
      <div className="mt-4 sm:mt-6">
        <h4 className="font-medium mb-3 text-sm sm:text-base">Available Translations</h4>
        <div className="flex flex-wrap gap-2">
          {version.translations.length > 0 ? (
            version.translations.map((lang, index) => (
              <Badge key={index} variant="outline" className="text-xs sm:text-sm">{lang}</Badge>
            ))
          ) : (
            <span className="text-xs sm:text-sm text-gray-500">No translations available</span>
          )}
        </div>
      </div>
      <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t">
        <h4 className="font-medium mb-3 text-sm sm:text-base">Recent Changes</h4>
        <div className="space-y-3">
          {version.changelog.map((log, index) => (
            <div key={index} className="text-xs sm:text-sm">
              <div className="flex flex-wrap items-center gap-2 text-gray-600">
                <Calendar className="h-3 w-3 sm:h-4 sm:w-4" />{log.date}
                <span className="text-gray-400 hidden sm:inline">|</span>
                <Users className="h-3 w-3 sm:h-4 sm:w-4" />{log.author}
              </div>
              <ul className="mt-1 space-y-1 ml-4 sm:ml-6 list-disc text-gray-600">
      ...
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4"  style={{ textAlign: 'left' }}>
        <div className="bg-white rounded-xl w-full max-w-4xl">
          <div className="p-4 sm:p-6 border-b">
            <div className="flex items-center justify-between">
              <h2 className="text-lg sm:text-xl font-semibold">Edit Contact Information - {info?.country || 'New Country'}</h2>
          ...
          <div className="p-4 sm:p-6 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.companyName} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">VAT Number</label>
                <input type="text" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.vatNumber} />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <input type="email" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.email} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Phone</label>
                <input type="tel" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.phone} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">WhatsApp</label>
                <input type="tel" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.whatsapp} />
              </div>
            </div>
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.address} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">City</label>
                  <input type="text" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.city} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Region/State</label>
                  <input type="text" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.region} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Postal Code</label>
                  <input type="text" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.postalCode} />
                </div>
              </div>
            </div>
            <div>
              <h3 className="text-base font-medium mb-3">Business Hours</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Weekday Hours</label>
                  <input type="text" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.businessHours?.weekday} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Weekend Hours</label>
                  <input type="text" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={info?.businessHours?.weekend} />
                </div>
              </div>
            </div>
    ...
    return (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4"  style={{ textAlign: 'left' }}>
        <div className="bg-white rounded-xl w-full max-w-2xl">
          <div className="p-4 sm:p-6 border-b">
            <div className="flex items-center justify-between">
              <h2 className="text-lg sm:text-xl font-semibold">{item ? 'Edit Menu Item' : 'Add Menu Item'}</h2>
              <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full">
                <X className="h-4 w-4 sm:h-5 sm:w-5" />
              </button>
            </div>
          ...
          <div className="p-4 sm:p-6 space-y-4">
            <div>
              ...
              <input type="text" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={item?.title} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">URL</label>
              <input type="text" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={item?.url} />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Location</label>
                <select className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={item?.location}>
              ...
              <div>
                ...
                <select className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={item?.visibility}>
            ...
            <div>
              ...
              <input type="number" className="w-full px-2 py-1 sm:px-3 sm:py-2 border rounded-lg text-sm" defaultValue={item?.order} min="1" />
            </div>
            <div className="pt-4 border-t flex justify-end gap-3">
              <Button variant="outline" onClick={onClose}>Cancel</Button>
              <Button>Save Changes</Button>
  ...
  const renderSocialContent = () => (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <h2 className="text-lg sm:text-xl font-semibold">Social Media URLs</h2>
        <Button onClick={() => setShowSocialModal(true)} className="w-full sm:w-auto text-sm">
          <Edit className="h-3 w-3 sm:h-4 sm:w-4 mr-2" />Edit URLs
        </Button>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:gap-6">
        <Card className="p-4 sm:p-6">
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              <Linkedin className="h-5 w-5 text-blue-800 flex-shrink-0" />
              <div className="flex-grow">
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-blue-50 rounded-lg">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm text-blue-700 font-medium">How Social Media URLs Are Used</p>
                  <p className="text-sm text-blue-600 mt-1">
                    These URLs will be displayed in the website footer, customer dashboard, and professional dashboard. 
                    They allow users to connect with Kokobeo on various social media platforms.
                  </p>
                </div>
              </div>
            </div>
            <div className="space-y-2">
              <h4 className="font-medium">Display Locations:</h4>
              <div className="flex flex-wrap gap-2">
                <Badge>Website Footer</Badge>
                <Badge>Customer Dashboard</Badge>
                <Badge>Professional Dashboard</Badge>
            ...
            <div className="mt-4 p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">Social Media Hashtag</span>
              </div>
              <div className="mt-1 text-sm">#Kokobeo</div>
            </div>
          </div>
        </Card>
      </div>
  ...
  const renderContactEditor = () => (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <h2 className="text-lg sm:text-xl font-semibold">Contact Information Management</h2>
        <Button onClick={() => setShowAddModal(true)} className="w-full sm:w-auto text-sm">
          <Plus className="h-3 w-3 sm:h-4 sm:w-4 mr-2" />Add Country
        </Button>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:gap-6">
        {Object.entries(contactInfo).map(([country, info]) => (
          <Card key={country} className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start">
              <div className="mb-4 sm:mb-0">
                <div className="flex items-center gap-2">
                  <Flag className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600" />
                  <h3 className="text-base sm:text-lg font-semibold capitalize">{country}</h3>
                </div>
                <div className="mt-2 text-xs sm:text-sm text-gray-600">{info.companyName} - VAT: {info.vatNumber}</div>
              </div>
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" onClick={() => setSelectedContactInfo({ country, ...info })}>
                  <Edit className="h-3 w-3 sm:h-4 sm:w-4" />
                </Button>
                <Button variant="ghost" size="sm" className="text-red-600">
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <h4 className="font-medium mb-2 text-sm sm:text-base">Business Hours</h4>
                <div className="space-y-2 text-xs sm:text-sm">
                  <div className="flex items-center gap-2">
                    <Clock className="h-3 w-3 sm:h-4 sm:w-4 text-gray-400" />
                    <span>Weekday: {info.businessHours.weekday}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Clock className="h-3 w-3 sm:h-4 sm:w-4 text-gray-400" />
                    <span>Weekend: {info.businessHours.weekend}</span>
                  </div>
                </div>
              </div>
            </div>
            <div className="mt-4">
              <h4 className="font-medium mb-2 text-sm sm:text-base">Address</h4>
              <div className="flex items-start gap-2 text-xs sm:text-sm">
                <MapPin className="h-3 w-3 sm:h-4 sm:w-4 text-gray-400 mt-1" />
                <div>
                  <div>{info.address}</div>
                  <div>{info.city}, {info.region} {info.postalCode}</div>
  ...
  const renderMenuEditor = () => (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <h2 className="text-lg sm:text-xl font-semibold">Menu & Footer Management</h2>
        <Button onClick={() => setShowAddModal(true)} className="w-full sm:w-auto text-sm">
          <Plus className="h-3 w-3 sm:h-4 sm:w-4 mr-2" />Add Menu Item
        </Button>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:gap-6">
        {menuItems.map((item) => (
          <Card key={item.id} className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start">
              <div className="mb-4 sm:mb-0">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <Badge variant="outline" className="text-xs sm:text-sm capitalize">{item.location}</Badge>
                  <Badge variant="outline" className="text-xs sm:text-sm capitalize">{item.visibility}</Badge>
                </div>
              </div>
              <div className="flex gap-2">
                <Button variant="ghost" size="sm" onClick={() => setSelectedMenuItem(item)}>
                  <Edit className="h-3 w-3 sm:h-4 sm:w-4" />
                </Button>
                <Button variant="ghost" size="sm" className="text-red-600">
                  <Trash2 className="h-3 w-3 sm:h-4 sm:w-4" />
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <h4 className="font-medium mb-2 text-sm sm:text-base">URL</h4>
                <div className="text-xs sm:text-sm text-gray-600">{item.url}</div>
              </div>
              <div>
                <h4 className="font-medium mb-2 text-sm sm:text-base">Display Order</h4>
                <div className="text-xs sm:text-sm text-gray-600">{item.order}</div>
              </div>
            </div>
          </Card>
        ))}
  ...
  const renderMainContent = () => {
    switch (activeTab) {
      ...
      default: 
        return <ContentEditor documentType={activeTab} />;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50"  style={{ textAlign: 'left' }}>
          ...
          <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-sm sm:text-base text-gray-600 mt-1">Manage legal documents, contact information, and menu items</p>
              </div>
              {(activeTab !== 'contact' && activeTab !== 'menu' && activeTab !== 'social') && (
                <Button onClick={() => setShowAddModal(true)} className="w-full sm:w-auto flex items-center gap-2 text-sm sm:text-base">
                  <Plus className="h-3 w-3 sm:h-4 sm:w-4" />New Document
                </Button>
              )}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              <Card className="p-4 sm:p-6">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-xs sm:text-sm text-gray-500">Active Documents</p>
              ...
              <Card className="p-4 sm:p-6">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-xs sm:text-sm text-gray-500">Total Versions</p>
              ...
              <Card className="p-4 sm:p-6">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-lg sm:text-2xl font-semibold mt-1">2d ago</p>
                  </div>
            ...
            <div className="flex overflow-x-auto gap-2 pb-2 snap-x">
              {documentTypes.map(type => (
                <button 
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg whitespace-nowrap snap-start text-sm sm:text-base ${activeTab === type.id ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}
                >
              ...
              {additionalTabs.map(tab => (
                <button 
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg whitespace-nowrap snap-start text-sm sm:text-base ${activeTab === tab.id ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}
                >
            ...
            <Card className="p-4">
              <div className="flex flex-col gap-4">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
                  <div className="relative w-full sm:min-w-[200px] md:min-w-[300px]">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  {(activeTab === 'customerInfo' || activeTab === 'professionalInfo') && (
                    <select 
            ...
            {(activeTab !== 'contact' && activeTab !== 'menu' && activeTab !== 'social') && (
              <div className="grid grid-cols-1 gap-4 sm:gap-6">
                {versions
                  .filter(version => (activeTab === 'all' || version.type === activeTab))
                  .filter(version => (selectedLanguage === 'all' || version.language.toLowerCase() === selectedLanguage))
                  .filter(version => (selectedCountry === 'all' || !version.country || version.country === countries.find(c => c.id === selectedCountry)?.name))
                  .map(version => (
      ...
      {(showAddModal && activeTab !== 'contact' && activeTab !== 'menu' && activeTab !== 'social') && (
        <DocumentModal version={selectedVersion} isOpen={true} onClose={() => {setShowAddModal(false); setSelectedVersion(null);}} />
      )}
      {(showAddModal && activeTab === 'contact') && !selectedContactInfo && (
        <ContactEditModal info={null} isOpen={true} onClose={() => setShowAddModal(false)} />
      )}
      {selectedContactInfo && (
        <ContactEditModal info={selectedContactInfo} isOpen={true} onClose={() => setSelectedContactInfo(null)} />
      )}
      {(showAddModal && activeTab === 'menu') && !selectedMenuItem && (
        <MenuItemModal item={null} isOpen={true} onClose={() => setShowAddModal(false)} />
      )}
      {selectedMenuItem && (
        <MenuItemModal item={selectedMenuItem} isOpen={true} onClose={() => setSelectedMenuItem(null)} />
      )}
      {showPageEditor && (
        <PageEditor 
          page={selectedPage} 
          onSave={(pageData) => {
            setPages(prev => ({
              ...
              [activeTab]: {
                ...prev[activeTab], 
                [pageData.url.split('/').pop()]: pageData
              }
            })); 
            setShowPageEditor(false); 
            setSelectedPage(null);
          }} 
          onClose={() => {setShowPageEditor(false); setSelectedPage(null);}} 
        />
      )}
      {showSocialModal && (
        <SocialMediaModal 
